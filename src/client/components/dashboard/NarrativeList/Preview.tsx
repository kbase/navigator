...
import { CellIcon, TypeIcon, AppCellIcon, DefaultIcon } from '../../generic/Icon';
...
export default class Preview extends Component<Props, State> {
    ...
    componentDidUpdate(prevProps: Props) {
        if (prevProps.narrative.access_group !== this.props.narrative.access_group ||
    ...
    fetchNarrativeObject() {
        ...
        return fetchNarrative(upa)
            .then(narr => {
                ...
                this.setState({isLoading: false, narrObj: narr, cells: cells, error: null});
            })
            .catch(error => {
    ...
    render() {
        ...
        const truncated = this.state.cells
              .slice(0, maxLength);
        ...
        return (
            <div className="flex flex-row flex-nowrap pv3 pl2">
                ...
                <div className="ml4" style={{minWidth: 0}}>
                    ...
                    <div className="black-80 f6 mt1 truncate" style={{}}>{this.props.subtitle}</div>
                </div>
            </div>
        );
    }
