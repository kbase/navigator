...
export const keepParamsLinkTo = (paramsToKeep: string[], link: string) => (
  ...
  const pathSafeCondition =
    loc.pathname[loc.pathname.length - 1] === '/' || '?#'.indexOf(link[0]) > -1;
  const pathnameNormalized = pathSafeCondition ? loc.pathname : pathnameTrimmed;
  const path = link[0] === '/' ? link : pathnameNormalized + link;
  const extraSlash = path[0] === '/' ? '' : '/';
  const newLink = new URL(location.origin + extraSlash + path);
