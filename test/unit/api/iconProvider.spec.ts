...
import IconProvider, { IconInfo, AppTag } from '../../../src/client/api/iconProvider';
...
test('Should have known type icons for known types', () => {
    ...
    ['KBaseGenomes.Genome-1.0', 'KBaseGenomes.Genome', 'Genome', 'kbasegenomes.genome'].forEach(t => {
        expect(ip.typeIcon(t)).toEqual(genomeIcon);
    });
});

test('Should have default app icons for unknown apps', async () => {
    ['SomeModule/SomeApp', 'SomeModule.SomeApp', 'SomeApp', ''].forEach(async app => {
...
test('Should have an app icon for a known app', async () => {
    const appIcon = await ip.appIcon('kb_megahit/run_megahit', 'release' as AppTag);
    expect(appIcon).toEqual(defaultApp);
});
