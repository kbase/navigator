...
import { getToken, getUsername, getUsernames, searchUsernames } from '../../../src/client/utils/auth';
enableFetchMocks();
...
describe('searchUsernames tests', () => {
    ...
    it('Should return a username map', async () => {
        ...
        expect(fetchMock.mock.calls[0][0]).toMatch(/auth\/api\/V2\/users\/search\/user$/);
        expect(users).toEqual(usermap);
    });
    ...
    it('Should include optional fields with URL formatting', async () => {
        ...
        expect(fetchMock.mock.calls.length).toEqual(1);
        expect(fetchMock.mock.calls[0][0]).toMatch(/auth\/api\/V2\/users\/search\/us\/\?fields=a,b,c$/);
        expect(users).toEqual(usermap);
    })
});
